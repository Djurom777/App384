...
class CoreDataStack {
    ...
    func deleteBudget(withBudTitle name: String, completion: @escaping () -> Void) {
        
        let context = CoreDataStack.shared.persistentContainer.viewContext
        let fetchRequest: NSFetchRequest<BudgetsModel> = BudgetsModel.fetchRequest()
        
        fetchRequest.predicate = NSPredicate(format: "budTitle == %@", name)
